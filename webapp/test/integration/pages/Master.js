		...
		onTheMasterPage : {
			...
			actions : {
				...
				iFilterTheListOnUnitNumber : function () {
					return this.iMakeASelection("filterButton", "<ProductID>", "<100 <CategoryName>");
				},

				iGroupTheList : function () {
					return this.iChooseASorter("groupButton", "<ProductID> Group");
				},
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("CategoryName") === oObject.CategoryName;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].CategoryName;
									} else {
										sCurrentId = aItemsNotInTheList[0].CategoryName;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Alphabetical_list_of_products", {
										CategoryName : sCurrentId
									});
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<Alphabetical_list_of_products> (0)"}),
						success: function () {
				...
				theListShouldBeFilteredOnUnitNumber : function () {
					return this.theListShouldBeFilteredOnFieldUsingComparator("ProductID", 100);
				},
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<Alphabetical_list_of_products> (" + iExpectedLength + ")"}),
								success : function () {
